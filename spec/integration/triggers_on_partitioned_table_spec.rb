    ...
    context "when postgresql 11 version and upper" do
      before(:all) do
        ...
        Dir.chdir("#{File.dirname(__FILE__)}/../dummy") do
          successfully "rails generate logidze:model partitioned_post --limit 4 --backfill"
        ...
        PartitionedPost.reset_column_information
      end

      after(:all) { @old_post.destroy! }

      let(:params) { {logdate: "2023-01-01", title: "Triggers", rating: 10, active: false, meta: {tags: %w[some tag]}, data: {some: "json"}} }

      describe "backfill" do
        let(:post) { PartitionedPost.find_by(id: @old_post.id, logdate: @old_post.logdate) }
      ...
      describe "insert" do
        let(:post) { PartitionedPost.create!(params).reload }
        ...
        context "when logging is disabled" do
          let(:post) { PartitionedPost.without_logging { PartitionedPost.create!(logdate: "2023-02-02").reload } }
      ...
      describe "update" do
        before(:all) { @post = PartitionedPost.create!(logdate: "2023-01-01") }
        ...
        it "creates several versions", :aggregate_failures do
          ...
          PartitionedPost.where(id: post.id).update_all(active: true)
        ...
        it "doesn't create new version if values not changed", :aggregate_failures do
          PartitionedPost.where(id: post.id).update_all(rating: nil)
        ...
        context "log_data is empty" do
          let(:post) { PartitionedPost.without_logging { PartitionedPost.create!(params).reload } }
        ...
        context "with unicode changes" do
          let(:post) { PartitionedPost.create!(params) }
      ...
      describe "undo/redo" do
        before(:all) { @post = PartitionedPost.create!(title: "Triggers", rating: 10, logdate: "2023-01-01") }
        after(:all) { @post.destroy! }
      ...
      describe "switch_to!" do
        before(:all) { @post = PartitionedPost.create!(title: "Triggers", rating: 10, logdate: "2023-01-01", data: {some: "json"}) }
        after(:all) { @post.destroy! }
      ...
      describe "limit" do
        before(:all) { @post = PartitionedPost.create!(title: "Triggers", logdate: "2023-01-01", rating: 10) }
        after(:all) { @post.destroy! }
      ...
      describe ".with_full_snapshot" do
        before(:all) { @post = PartitionedPost.create!(title: "Triggers", rating: 10, logdate: "2023-02-01") }
        after(:all) { @post.destroy! }
