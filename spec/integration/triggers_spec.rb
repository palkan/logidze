...
describe "triggers", :db do
  [Post, DetachedPost].each do |klass|
    describe "with #{(klass == Post) ? "in-place" : "detached"} log_data storage" do
      ...
      describe "backfill" do
        let(:post) { klass.find(@old_post.id) }
      ...
      describe "insert" do
        let(:post) { klass.create!(params).reload }
        ...
        context "when logging is disabled" do
          let(:post) { klass.without_logging { klass.create!.reload } }
      ...
      describe "diff" do
        ...
        it "generates the correct diff on fallback", :aggregate_failures do
          ...
          snapshot = if klass == Post
            JSON.parse(post.read_attribute_before_type_cast(:log_data))
          else
            JSON.parse(post.logidze_data.read_attribute_before_type_cast(:log_data))
          end
      ...
      describe "update" do
        ...
        it "creates several versions", :aggregate_failures do
          post.update!(params)
          # Since we don't cache association for log_data stored in a separate table, we should return reloaded `log_version`
          expect(post.log_version).to eq 1 if klass == Post
          ...
          klass.where(id: post.id).update_all(active: true)
        ...
        it "doesn't create new version if values not changed", :aggregate_failures do
          klass.where(id: post.id).update_all(rating: nil)
        ...
        context "log_data is empty" do
          let(:post) { klass.without_logging { klass.create!(params).reload } }

          it "creates several versions", :aggregate_failures do
            post.update!(rating: 0)
            post.update!(title: "Updated")
            # Since we don't cache association for log_data stored in a separate table, we should return relevant `log_data`
            expect(post.log_data).to be_nil if klass == Post
      ...
      describe "undo/redo" do
        before(:all) { @post = klass.create!(title: "Triggers", rating: 10) }
        after(:all) { @post.destroy! }
      ...
      describe "switch_to!" do
        before(:all) { @post = klass.create!(title: "Triggers", rating: 10, data: {some: "json"}) }
        after(:all) { @post.destroy! }
