...
describe Logidze::Model, :db do
  [User, DetachedUser].each do |klass|
    describe "For log_data stored #{(klass == User) ? "in record's origin table" : "in a separate table"}" do
      let(:user) do
        user = klass.new(
          name: "test",
          age: 10,
        ...
        user.build_logidze_data if klass == DetachedUser
        user.log_data = {
          "v" => 5,
          "h" =>
            [
      ...
      describe "undo!" do
        ...
        context "with no possible undo" do
          let(:new_user) do
            user = klass.new
            user.build_logidze_data if klass == DetachedUser
            user.log_data = {
          ...
          it "returns false" do
            expect(new_user.undo!).to eq false
          end
        end
      ...
      describe ".at" do
        ...
        it "returns reverted records", :aggregate_failures do
          u = klass.at(time: time(350)).first
        ...
        it "returns reverted records when called on relation", :aggregate_failures do
          u = klass.where(active: false).order(age: :desc).at(time: time(350)).first
        ...
        it "skips nil records" do
          ...
          expect(klass.at(time: time(350)).size).to eq 1
        end
      end
      ...
      describe ".diff_from" do
        ...
        it "returns diffs for records", :aggregate_failures do
          expect(
            klass.diff_from(time: time(350)).first
          ).to eq(
            "id" => user.id,
            "changes" =>
              {
        ...
        it "returns diffs for records when called on relation", :aggregate_failures do
          expect(
            klass.where(active: false).order(age: :desc).diff_from(time: time(350)).first
          ).to eq(
            "id" => user.id,
            "changes" =>
              {
      ...
      describe ".reset_log_data" do
        let!(:user2) do
          user.dup.tap {
            _1.build_logidze_data if klass == DetachedUser
            _1.log_data = user.log_data
            _1.save!
          }
        end
        let!(:user3) do
        ...
        before { klass.limit(2).reset_log_data }
        ...
        if klass == DetachedUser
          let(:other_user) do
            other_user = DetachedOtherUser.new(
              name: "test",
              age: 10,
              active: false
            )
            other_user.build_logidze_data
            other_user.log_data = user.log_data.dup
            other_user.save!
            other_user
          end
